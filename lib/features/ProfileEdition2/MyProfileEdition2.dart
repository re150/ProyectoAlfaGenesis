...
class ProfileEdition2 extends StatefulWidget {
  const ProfileEdition2({super.key});

  @override
  State<ProfileEdition2> createState() => _ProfileEdition2State();
}

class _ProfileEdition2State extends State<ProfileEdition2> {
  ...
  Future<void> editProfile(String Urlimg) async {
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    ...
    try {
      final response = await http.patch(
        ...
        body: jsonEncode(
            {"name": dataProfile.name, "imgUrl": Urlimg, "id": dataProfile.id}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        mostrarMensajeError(
            'Error en la edición del perfil. Código de estado: ${response.statusCode}');
      }
    } catch (e) {
                      ...
                      child: GridView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return MyProfileImageButton(
                              ...
                              onTap: () {
                                setState(() {
                                  if(index*10<=puntajeTotal) selectedImage = imagePaths[index];
                                });
                              },
                            );
                          })),
          ...
          Expanded(
            ...
            child: Container(
              ...
              child: MyButton(
                ...
                colorT: Colors.white,
                onTap: () {
                  editProfile("assets/ProfilePictures/$selectedImage");
                },
              ),
            ),
          ),
