...
class _MyGroupCreationPageState extends State<MyGroupCreationPage> {
                ...
                setState(() {
                  if (_grupos[numero - 1]!.lista.isNotEmpty) {
                    for (var alumno in _grupos[numero - 1]!.lista) {
                      _deleteAlumno(
                          _grupos[numero - 1]!.id, alumno.id, alumno.name);
  ...
  void _usargrupo(String id, List<Alumno> listaAlumnosGrupo) async {
    ...
    if (teamOnDB.isNotEmpty && listaAlumnosGrupo.isNotEmpty) {
      if (teamOnDB.containsKey(id)) {
        setState(() {
          _indexIndicado =
              _grupos.indexWhere((element) => element!.nombreGrupo == id);
          _seleccionarGrupo(_indexIndicado + 1);
          Provider.of<TeamProvider>(context, listen: false)
              .setIdTeam(teamOnDB[id]!);
  ...
  Future<void> _loadGrupos() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        List<Map<String, dynamic>> jsonTeam =
  ...
  void _updateTeamMembers(List<Alumno> alumnos, String name, int numTeam) {
    ...
    for (var alumno in alumnos) {
      for (var i = 0; i < _alumnos.length; i++) {
        if (alumno.name == _alumnos[i].name &&
            alumno.id == _alumnos[i].id &&
            !_alumnos[i].teamStatus) {
  ...
  void _agregarAlumnoAlGrupo(Alumno alumno) {
    setState(() {
      _addAlumno(alumno, _grupos[_indexIndicado]!.id,
          _grupos[_indexIndicado]!.lista.length + 1);
    ...
    for (var profileString in profiles) {
      ...
      for (var match in matches) {
        ...
        _alumnos.add(Alumno.fromMap({
          ...
          'grupo': match.group(5)!,
          'teamStatus':
              match.group(8) == 'null' ? null : match.group(7) == 'true'
        }));
      }
    }
  }
  ...
  List<Alumno> _parseProfilesToList(List<Map<String, dynamic>> profiles) {
    ...
    for (var profile in profiles) {
      ...
      bool? teamStatus = profile['teamStatus'] == 'null'
          ? null
          : profile['teamStatus'] == 'true';
